...
export default function CourseInfo() {
  ...
  const [courseTeacher, setcourseTeacher] = useState<Creator>();

  const [sessions, setSessions] = useState<SessionsEntity[] | []>([]);
  const [CourseDetail, setCourseDetail] = useState<Weather>();
  const [relatedCourses, setRelatedCourses] = useState<relatedCourses[]>([]);

  const mainNameCourse = courseName?.split("-")[0];
  ...
  const handleCommentSubmission = (
    ...
    fetch(`http://localhost:4000/v1/comments`, {
      ...
      body: JSON.stringify({
        body: newCommentBody,
        courseShortName: mainNameCourse,
        score: commentScore,
      }),
    }).then((res) => {
      if (res.ok) {
  ...
  const registerInCourse = (course: Weather | undefined) => {
    if (course?.price === 0) {
      swal({
                ...
                .then((code) => {
                  ...
                  fetch(
                    `http://localhost:4000/v1/courses/${course?._id}/register`,
                    {
                      ...
                      body: JSON.stringify(resultePrice),
                    }
                  ).then((res) => {
                    console.log(res);
                    if (res.ok) {
